...
- 匹配、Room、权威服务器与联机：按本文件公布的 Builda Online 能力状态分流，不得恢复旧 Backend Pack。
...
当前部署没有配置 Builda Online 联机入口，代表联机能力尚未上线。用户要求匹配、Room、权威服务器、
本地联机调试或 Room 部署时，明确告知“联机功能内测中，如有需求请联系builda官方”，不要尝试安装联机工具链、
恢复已删除的 `builda online pack/build/deploy` 或旧 Backend Pack，也不得让用户手工复制任何 token
绕过当前部署的能力状态。
...
- agent grant 有效期 30 分钟，只能进入发布链（含由 Builda Online 部署客户端为自己游戏换一次性 Online deploy code）；它不是完整登录态，也不能直接调用 Online。
...
凡是任务涉及云存档、支付订单去重、排行榜或其他由游戏构造的持久化标识符，必须先读取 `https://ai.builda.game/agent/releases/0.4.34/md_persistent_ids.md`。从 `0.3.1` 起，privateKV key 接受 `^[A-Za-z0-9_:-]{1,64}$`，`payId`、`rankId` 接受 `^[A-Za-z0-9_-]{1,64}$`。新代码统一只使用字母、数字、`_`、`-`，但 `save:main` 等存量 privateKV 冒号 key 可以继续读写删，不得仅因冒号要求迁移或阻断草稿。真正非法的旧数据默认保留，禁止自动 remove。修改 `payId` 会改变商品映射，修改 `rankId` 等价于新榜，执行前必须取得用户明确确认。
...
服务 token：Builda Online 等游戏逻辑层直连服务使用 `Builda.requestToken()`（Godot 侧 `builda.request_token()`，Unity 侧 `Builda.RequestToken(callback)`）换取短期凭证，返回 `{token, expiresAt}`（`expiresAt` 为 Unix 秒）。token 绑定当前游戏 × 当前玩家，由 Builda 平台 extGame 签发、Online 通过平台接口校验；只在内存持有，**严禁**写入 privateKV、本地文件、存档、URL 或日志。临近 `expiresAt` 时重新请求。它不是 Room 部署凭证。本地 `builda dev` 的 mock token 不能连接真实 Online，只用于跑通过期刷新逻辑。联机功能内测中，如有需求请联系builda官方；不要把 mock token 用于连接真实联机服务。
...
多语言：游戏若做本地化，用 `Builda.runtime.language()`（Godot 侧 `builda.runtime_language()`）取当前语言，返回 BCP 47 连字符形态的字符串，同步方法、直接返回裸字符串（不需要 await、没有 `ok` 包装），取不到时兜底 `en`、绝不返回空串。平台只给**裸语言码**（`zh`、`en`，不带区域变体，不会出现 `zh-CN`），唯一例外西语为 `es-419`——不要做简繁分支或按区域细分文案的设计。匹配自己语言包时仍按"精确匹配 → 语言前缀降级（如 `es-419` → `es`）→ 游戏默认语言"三级回退，不要只做全等比较。启动时读一次决定加载哪套文案即可；宿主支持运行中切语言，在切场景等节点重读能跟上变化，无需轮询。本地 `builda dev` 的 mock 把浏览器 `navigator.language` 收敛到同一粒度（如 `zh-CN` → `zh`、`es-MX` → `es-419`），测指定语言可在游戏 URL 加 `builda_mock_lang=es-419` 这类参数覆盖。用户要做多语言或遇到 Web 端文字变方块（tofu）时，先读完整实践指南：`https://ai.builda.game/agent/releases/0.4.34/md_godot_i18n.md`（内嵌文案表、字体子集裁剪、排行榜玩家名等开集文本的缺字过滤、日俄长文案排版）。字体口径按引擎区分：Godot Web 导出（wasm 自绘）没有系统字体回退，字体由游戏自行处理并随包分发——全量 CJK 字体禁止直接打进包（用 fontTools 裁子集），也禁止从任何外部 CDN 加载字体（平台不提供共享字体，COEP 下外链会被拦截白屏）；Unity WebGL 同类（TMP 与 legacy Font 都无系统回退，字体内嵌且裁子集；TMP/legacy 选型、i18n 文案表、开集文本缺字过滤见 `https://ai.builda.game/agent/releases/0.4.34/md_unity_webgl.md` 第 5 章——**编辑器 Game 视图有系统字体回退，字体验收必须在 Web 构建里做**）；HTML5 渲染的游戏（pixi/phaser/canvas）相反，**优先直接用系统字体**（见「HTML5 打包」），不要照搬 Godot 的字体裁剪流程。
...
用户需要新增、修改或清空排行榜时，先读取完整契约：`https://ai.builda.game/agent/releases/0.4.34/md_rankboards.md`。
...
- 已走 `res/**` 的资源**不要再打进主包 zip**（体积白付两遍）；本地开发时把 `res/` 目录拷进构建目录，`builda dev` 下同源可取，正式包里删掉。mock SDK 在无 `assetsBaseUrl` 时会把 `assets.url()` 按页面所在目录解析成**绝对 URL**，Godot `HTTPRequest`/Unity `UnityWebRequest` 可直接使用，与真机行为一致。
...
用户抱怨包体太大、上传超限或想做"资源分包/动态加载"时，先读完整专题：`https://ai.builda.game/agent/releases/0.4.34/md_bundle_size.md`（诊断方法、外置决策、音频/字体/纹理/引擎导出全套瘦身手段、上传前检查清单，以及 Godot 资源分包实战避坑——Web 下载落盘、静态引用链、preset 生成、按需加载审计）。
...
默认 manifest 故意省略 `rankBoards`，表示普通更新保持已有排行榜。需要配置排行榜时按 `https://ai.builda.game/agent/releases/0.4.34/md_rankboards.md` 添加完整数组；只有用户明确要求清空全部榜单时才写 `"rankBoards": []`。
...
Builda Online Room 只在用户明确要求时按“Builda Online 联机”章节转交处理；本 CLI 不含任何 Room 部署命令。
...
- 涉及 privateKV、支付或排行榜改动时，按 `https://ai.builda.game/agent/releases/0.4.34/md_persistent_ids.md` 核对稳定标识符设计；不要随意改动已有 `privateKV key`、`payId` 或 `rankId`。
...
Unity 项目（engine=unity）上传的是 **WebGL 构建输出**，不是 Unity 工程。完整导出专题（含常见坑）见 `https://ai.builda.game/agent/releases/0.4.34/md_unity_webgl.md`，要点：
...
4. 字体：WebGL 下 TMP 与 legacy Font 都没有系统字体回退（编辑器里有——Game 视图效果不可信），CJK 文本必须内嵌字体且用 fontTools 裁剪子集，全量 CJK 字体不要直接入包；排行榜玩家名等开集文本要做运行时缺字过滤。做多语言或字体前先读 `https://ai.builda.game/agent/releases/0.4.34/md_unity_webgl.md` 第 5 章（TMP/legacy 选型、includeFontData 陷阱、裁剪与缺字过滤全流程）。
...
- Room 云构建/部署请求：联机功能内测中，如有需求请联系builda官方；直接说明能力状态，不要尝试恢复旧命令或用任何 token 手工调用部署接口。
